...
public class LayDownFire : ReactionAbility {
    ...
    public override bool TriggersReaction(Tile tile, Actor actor) {
        return owner.shotsRemaining > 0 && !tile.foggy && owner.InRange(tile.gridLocation) && owner.WithinSightArc(tile.gridLocation) && owner.CanSee(tile.gridLocation) && actor is Alien;
    }
    ...
    IEnumerator PerformShots() {
        ...
        bool woundUp = false;
        if (owner.shotsRemaining > 0 && aliensInSight.Count > 0) {
            woundUp = true;
            owner.AimAnimation();
        ...
        while (owner.shotsRemaining > 0 && aliensInSight.Count > 0) {
            if (owner.shotsRemaining >= owner.shots) owner.shotsSpent += 1;
        ...
        if (owner.shotsRemaining <= 0) owner.HideAbilityIcon();
        if (woundUp) {
